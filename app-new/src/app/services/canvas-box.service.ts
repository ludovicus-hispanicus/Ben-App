...
export class CanvasBoxService {
  ...
  setupWheelZoomPan(canvas: fabric.Canvas, config: ZoomPanConfig): void {
    canvas.on('mouse:wheel', (opt) => {
      ...
      // Detect if this is likely a two-finger scroll vs zoom gesture
      // Two-finger scroll: has deltaX (horizontal movement component)
      // Zoom (Ctrl+wheel or pinch): no deltaX, only deltaY
      const isLikelyTwoFingerScroll = Math.abs(evt.deltaX) > 0;

      // Zoom on Ctrl+wheel or pinch (ctrlKey without horizontal movement)
      if ((evt.ctrlKey || evt.metaKey) && !isLikelyTwoFingerScroll) {
        const delta = evt.deltaY;
        let zoom = canvas.getZoom();
        // Smooth zoom: clamp delta to avoid large jumps from trackpad/wheel
        const clampedDelta = Math.max(-10, Math.min(10, delta));
        zoom *= 0.995 ** clampedDelta;
        zoom = this.clampZoom(zoom, config);
        canvas.zoomToPoint({ x: evt.offsetX, y: evt.offsetY } as fabric.Point, zoom);
        if (config.onZoomChange) config.onZoomChange(zoom);
        return;
      }

      // Everything else = pan (two-finger scroll, plain wheel, etc.)
      // Shift+wheel = horizontal scroll (convert deltaY to deltaX)
