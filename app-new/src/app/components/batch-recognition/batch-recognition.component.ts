...
export class BatchRecognitionComponent implements OnInit, OnDestroy {
  ...
  destinationProjectId: number | null = null;
  ...
  // CuReD project list (for destination badges)
  curedProjects: ProjectPreview[] = [];
  ...
  onDestinationProjectChange(projectId: number): void {
    this.destinationProjectId = projectId;
    const project = this.curedProjects.find(p => p.project_id === projectId);
    this.destinationProjectName = project ? project.name : '';
  }
  ...
  get hasDestination(): boolean {
    if (this.destinationMode === 'library') return !!this.destinationProjectId;
  ...
  createNewProject(): void {
    ...
    this.projectService.create(name).subscribe({
      next: (projectId: number) => {
        ...
        this.newProjectName = '';
        this.destinationProjectId = projectId;
        this.destinationProjectName = name;
        this.loadCuredProjects();
        this.notificationService.showSuccess(`Project "${name}" created`);
      },
      error: (err) => {
        this.isCreatingProject = false;
        this.notificationService.showError('Failed to create project: ' + (err.error?.detail || err.message));
      }
    });
  }

  private loadCuredProjects(): void {
    this.projectService.list().subscribe({
      next: (projects) => {
        this.curedProjects = projects;
      },
      error: () => {}
    });
  }
    ...
    if (!this.hasDestination) {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        data: {
          ...
          message: 'No destination project selected. Resulting texts will be unassigned and won\'t appear under any project in CuReD. Continue anyway?',
  ...
  private _doStartBatch(): void {
    ...
    const request: BatchRecognitionRequest = {
      ...
      destination_project_id: this.destinationMode === 'library' && this.destinationProjectId ? this.destinationProjectId : undefined,
  ...
  private _doContinueBatch(prevJob: BatchRecognitionStatus): void {
    ...
    const request: BatchRecognitionRequest = {
      ...
      destination_project_id: prevJob.destination_project_id,
