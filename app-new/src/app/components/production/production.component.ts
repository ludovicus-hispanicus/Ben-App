...
export class ProductionComponent implements OnInit, OnDestroy {
    ...
    availableProjects: ProjectPreview[] = [];
    ...
    applyFilter(): void {
        ...
        const { identifierTypes, labels, curation, projectId, dateRange, status } = this.activeFilters;
        ...
        this.filteredGroups = this.groupedTexts.filter(g => {
            ...
            if (projectId !== null) {
                if (projectId === -1) {
                    ...
                    if (!g.parts.some(p => !p.project_id)) return false;
                } else {
                    if (!g.parts.some(p => p.project_id === projectId)) return false;
                }
            }
    ...
    toggleProject(projectId: number): void {
        this.activeFilters.projectId = this.activeFilters.projectId === projectId ? null : projectId;
        this.applyFilter();
    }
    ...
    hasActiveFilters(): boolean {
        return this.activeFilters.identifierTypes.size > 0 ||
            ...
            this.activeFilters.projectId !== null ||
    ...
    getProjectName(projectId: number): string {
        if (projectId === -1) return 'Unassigned';
