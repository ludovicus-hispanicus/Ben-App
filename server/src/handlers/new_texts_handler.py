...
from mongo.local_db_client import LocalDBClient as MongoClient, MongoCursor, ShardedCollection
...
class NewTextsHandler:
    ...
    def __init__(self):
        ...
        db_dir = os.path.join(storage_path, "db")
        self._texts_dir = os.path.join(db_dir, "texts_by_dataset")
        self._index_path = os.path.join(db_dir, "text_index.json")

        # Migrate from single new_texts.json to per-dataset shards (one-time)
        old_path = os.path.join(db_dir, "new_texts.json")
        if not os.path.isdir(self._texts_dir) and os.path.exists(old_path):
            ShardedCollection.migrate_from_single_file(old_path, self._texts_dir, self._index_path)

        self._collection = ShardedCollection(self._texts_dir, self._index_path)
    ...
    def _parse_texts(self, results: List[Dict]) -> List[NewText]:
        """Parse raw dicts into NewText objects."""
        return [NewText.parse_obj(r) for r in results] if results else []

    def _parse_text(self, result: Dict) -> Optional[NewText]:
        """Parse a single raw dict into NewText."""
        return NewText.parse_obj(result) if result else None

    # ─── Persistent Training Data ─────────────────────────────────────

    def persist_kraken_training_data(self, text_id: int, image_path: str, lines: List[str], boxes: List[dict], dataset_id: int = None):
        """Crop individual lines from image and save as PNG + .gt.txt in kraken-training/.
        When dataset_id is set, saves to kraken-training/{dataset_id}/ subdirectory.
        ...
        self._cleanup_kraken_training(text_id, dataset_id)
        ...
        if dataset_id is not None:
            output_dir = os.path.join(self.KRAKEN_TRAINING_DIR, str(dataset_id))
        else:
        ...
        logging.info(f"persist_kraken: text {text_id} (dataset={dataset_id}) → {exported} line crops saved")
        return exported

    def cleanup_training_data(self, text_id: int, dataset_id: int = None):
        """Remove all persisted training data for a given text."""
        self._cleanup_kraken_training(text_id, dataset_id)

    def _cleanup_kraken_training(self, text_id: int, dataset_id: int = None):
        if dataset_id is not None:
            base_dir = os.path.join(self.KRAKEN_TRAINING_DIR, str(dataset_id))
        else:
    ...
    def get_stats_per_dataset(self) -> dict:
        ...
        """
        raw = self._collection.get_stats_per_dataset()
        # Convert string keys to int for consumer compatibility
        return {int(k): v for k, v in raw.items()}

    def list_texts(self, limit: int = 1000) -> List[NewTextPreviewDto]:
        results = self._collection.find_many(find_filter={}, limit=limit, sort=[("use_start_time", pymongo.DESCENDING)])
    ...
    def list_unassigned_texts(self) -> List[NewTextPreviewDto]:
        # Read directly from the "unassigned" shard
        shard = self._collection._get_shard(ShardedCollection.UNASSIGNED)
        results = shard.find_many(find_filter={}, limit=1000, sort=[("use_start_time", pymongo.DESCENDING)])
        texts = self._parse_texts(results)
        return [NewTextPreviewDto.from_new_text(new_text=t) for t in texts]
    ...
    def export_dataset_texts(self, dataset_id: int) -> List[dict]:
        """Export all texts in a dataset with full content."""
        results = self._collection.find_many(
            find_filter={"dataset_id": int(dataset_id)},
            limit=10000,
            sort=[("use_start_time", pymongo.DESCENDING)]
    ...
    def build_cured_export_zip(self, texts: List[NewText], dataset_id: int = None, project_name: str = None) -> str:
    ...
    def import_cured_zip(self, zip_path: str, target_dataset_id: int = None, uploader_id: str = "admin") -> dict:
    ...
    def import_cured_folder(self, folder_path: str, target_dataset_id: int = None, uploader_id: str = "admin") -> dict:
    ...
    def assign_text_to_dataset(self, text_id: int, dataset_id: int):
        self._collection.move_text(int(text_id), dataset_id)

    def unassign_texts_from_dataset(self, dataset_id: int):
        results = self._collection.find_many(find_filter={"dataset_id": int(dataset_id)}, limit=10000)
        texts = self._parse_texts(results)
        for text in texts:
            self.assign_text_to_dataset(text_id=text.text_id, dataset_id=None)

    def get_random_texts(self) -> List[NewTextPreviewDto]:
        results = self._collection.find_many(find_filter={}, limit=50, sort=[("use_start_time", pymongo.DESCENDING)])
        texts = self._parse_texts(results)
        return self.generate_previews(result=texts)
    ...
    def get_by_symbol(self, symbol: str) -> List[NewTextPreviewDto]:
        ...
        # and return the text
        results = self._collection.find_many(limit=100, find_filter={
            "transliterations": {
                "$elemMatch": {
                    "edit_history": {
                        "$elemMatch": {
    ...
    def search_kwic(self, query: str, limit: int = 200) -> list:
        ...
        # Load all texts
        results = self._collection.find_many(find_filter={}, limit=1000)
        all_texts = self._parse_texts(results)
        if not all_texts:
            return []
    ...
    def save_new_transliteration(self, dto: TransliterationSubmitDto, uploader_id: str):
        ...
        # Rebuild stats if curation changed
        if dto.is_curated_kraken or dto.is_curated_vlm or dto.is_fixed:
            self._collection.rebuild_stats()
        ...
        if dto.is_curated_kraken:
            ...
            if image_path:
                ...
                lines = dto.lines or []
                if len(boxes) == len(lines):
                    # Look up dataset_id for per-dataset storage
                    text_doc = self._collection.find_one({"text_id": dto.text_id})
                    proj_id = text_doc.get("dataset_id") if text_doc else None
                    self.persist_kraken_training_data(dto.text_id, image_path, lines, boxes, dataset_id=proj_id)
    ...
    def batch_curate(self, text_ids: List[int], curate: bool, target: str = "both", user_id: str = "admin") -> dict:
        ...
        Optimised: processes per-shard for efficiency.
        """
        updated = 0
        skipped = 0
        errors = []
        ...
        # Group text_ids by shard for efficient per-shard processing
        shard_groups: Dict[str, set] = {}
        for tid in id_set:
            tid_str = str(tid)
            did = self._collection._index.get(tid_str)
            key = self._collection._shard_key(did)
            shard_groups.setdefault(key, set()).add(tid)

        for shard_key, shard_ids in shard_groups.items():
            shard = self._collection._get_shard(shard_key)
            shard_updated = 0
            ...
            with shard._lock:
                ...
                for item in data:
                    ...
                    if tid not in shard_ids:
                        continue
                    shard_ids.discard(tid)
                ...
                skipped += len(shard_ids)  # remaining ids not found in this shard

                if shard_updated > 0:
                    shard._write_unsafe(data)

            updated += shard_updated
        ...
        # Rebuild stats cache since curation flags changed
        if updated > 0:
            self._collection.rebuild_stats()
    ...
    def delete_texts_batch(self, text_ids: List[int]) -> dict:
        """Delete multiple texts in a single read/write cycle per shard."""
        if not text_ids:
        ...
        for shard_key, shard_ids in shard_groups.items():
            ...
            with shard._lock:
                ...
                for doc in data:
                    if doc.get("text_id") not in shard_ids:
                        continue
                    for trans in doc.get("transliterations", []):
                ...
                data = [doc for doc in data if doc.get("text_id") not in shard_ids]
                deleted_count += original_count - len(data)

                shard._write_unsafe(data)

        # Remove from index
        with self._collection._index_lock:
            for tid in id_set:
                self._collection._index.pop(str(tid), None)
            self._collection._save_index()

        # Rebuild stats cache after batch delete
        if deleted_count > 0:
            self._collection.rebuild_stats()
    ...
    def remove_tile_markers(self, dataset_id: int = None, text_ids: List[int] = None) -> dict:
        """Remove ************************ tile merge marker lines from texts.
        If dataset_id is given, processes all texts in that dataset.
        ...
        if dataset_id is not None:
            shard_keys = [self._collection._shard_key(dataset_id)]
        elif text_ids:
            # Group by shard
            keys = set()
            for tid in text_ids:
                did = self._collection._index.get(str(tid))
                keys.add(self._collection._shard_key(did))
            shard_keys = list(keys)
        else:
            shard_keys = self._collection._all_shard_keys()

        text_id_set = set(text_ids) if text_ids else None
        ...
        for shard_key in shard_keys:
            ...
            with shard._lock:
                ...
                for doc in data:
                    if text_id_set and doc.get("text_id") not in text_id_set:
                        continue
    ...
    def get_curated_training_stats(self, target: str = None, dataset_id: int = None, dataset_ids: list = None) -> dict:
        """Get statistics about curated texts for training.
        If target is specified ("vlm" or "kraken"), only count texts with that target.
        If dataset_ids is specified, only count texts in those datasets.
        If dataset_id is specified (legacy), only count texts in that dataset.
        ...
        if dataset_ids:
            query["dataset_id"] = {"$in": [int(pid) for pid in dataset_ids]}
    ...
    def get_curated_training_data(self, target: str = None, dataset_id: int = None, dataset_ids: list = None) -> list:
        """
        Get curated training data, optionally filtered by target ("vlm" or "kraken")
        and/or dataset_id(s). Returns a list of dicts with image_path, lines, and boxes.
        """
        from utils.storage_utils import StorageUtils
        ...
        logging.info(f"Found {len(training_data)} curated texts for training (target={target}, dataset_ids={dataset_ids or dataset_id})")
        return training_data

    def get_curated_training_data_for(self, target: str, dataset_id: int = None, dataset_ids: list = None) -> list:
        """
        Get curated training data filtered by training target and optionally dataset(s).
        target: "vlm" or "kraken"
        """
        return self.get_curated_training_data(target=target, dataset_id=dataset_id, dataset_ids=dataset_ids)
