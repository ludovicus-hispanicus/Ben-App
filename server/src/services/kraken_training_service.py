...
class KrakenTrainingService:
    ...
    def get_training_stats(self, texts_handler, dataset_id: int = None, dataset_ids: list = None) -> dict:
        ...
        curated_stats = texts_handler.get_curated_training_stats(target=None, dataset_id=dataset_id, dataset_ids=dataset_ids)
    ...
    def export_training_data(self, texts_handler, dataset_id: int = None, dataset_ids: list = None) -> dict:
        """
        Export curated texts as training data (line images + ground truth).
        Uses pre-persisted data from kraken-training/ (or per-dataset subdir) if available,
        falls back to on-the-fly generation from DB.
        Supports multiple dataset_ids: aggregates data from all specified datasets.
        """
        # If multiple dataset_ids, aggregate data from each
        if dataset_ids and len(dataset_ids) > 1:
            return self._export_multi_project(texts_handler, dataset_ids)

        # Single dataset (or all datasets)
        effective_pid = dataset_ids[0] if dataset_ids and len(dataset_ids) == 1 else dataset_id
        logging.info(f"Exporting training data from curated texts (dataset_id={effective_pid})...")
        ...
        curated_data = texts_handler.get_curated_training_data_for("kraken", dataset_id=effective_pid)
    ...
    def _export_multi_project(self, texts_handler, dataset_ids: list) -> dict:
        """Aggregate training data from multiple datasets into a shared directory."""
        ...
        for did in dataset_ids:
            result = self.export_training_data(texts_handler, dataset_id=did)
        ...
        logging.info(f"Merged training data from {len(dataset_ids)} datasets: {total_lines} lines, {total_texts} texts")
        return {
    ...
    async def start_training(
        ...
        try:
            ...
            export_result = self.export_training_data(texts_handler, dataset_id=dataset_id, dataset_ids=dataset_ids)
