...
async def get_curated_stats(dataset_id: int = None):
    """Get curated data stats broken down by target (kraken vs vlm) plus totals.
    Optionally filtered by dataset_id."""
    all_stats = global_new_text_handler.get_curated_training_stats(target=None, dataset_id=dataset_id)
    kraken_stats = global_new_text_handler.get_curated_training_stats(target="kraken", dataset_id=dataset_id)
    vlm_stats = global_new_text_handler.get_curated_training_stats(target="vlm", dataset_id=dataset_id)
...
async def get_kraken_training_status(dataset_id: int = None, dataset_ids: Optional[List[int]] = Query(None)):
    ...
    # Normalise: single dataset_id → list for consistency
    dids = dataset_ids or ([dataset_id] if dataset_id is not None else None)

    # Get stats with previous/new breakdown
    stats = kraken_training_service.get_training_stats(global_new_text_handler, dataset_ids=dids)
...
async def start_kraken_training(epochs: int = 500, model_name: str = None, base_model: str = None, batch_size: int = 1, device: str = "auto", patience: int = 10, dataset_id: int = None, dataset_ids: Optional[List[int]] = Query(None)):
    """Start Kraken OCR training, optionally scoped to one or more datasets."""
    from services.kraken_training_service import kraken_training_service, TrainingStatus
    import asyncio
...
async def get_qwen_training_status(dataset_id: int = None, dataset_ids: Optional[List[int]] = Query(None)):
    ...
    stats = qwen_training_service.get_training_stats(global_new_text_handler, dataset_ids=dids)
...
async def start_qwen_training(
    ...
    patience: int = 3,
    dataset_id: int = None,
    dataset_ids: Optional[List[int]] = Query(None),
):
    """Start Qwen QLoRA fine-tuning, optionally scoped to one or more datasets."""
    from services.qwen_training_service import qwen_training_service
    from services.training_common import TrainingStatus
    import asyncio
...
async def get_trocr_training_status(dataset_id: int = None, dataset_ids: Optional[List[int]] = Query(None)):
    """Get TrOCR training status and data readiness."""
    from services.trocr_training_service import trocr_training_service
    from services.training_common import TrainingStatus

    dids = dataset_ids or ([dataset_id] if dataset_id is not None else None)
    stats = trocr_training_service.get_training_stats(global_new_text_handler, dataset_ids=dids)
...
async def start_trocr_training(
    ...
    """Start TrOCR fine-tuning, optionally scoped to one or more datasets."""
    from services.trocr_training_service import trocr_training_service
    from services.kraken_training_service import kraken_training_service
    from services.training_common import TrainingStatus
    import asyncio
...
async def export_dataset_cured(dataset_id: int):
    """Export all texts in a dataset as a CuReD zip (manifest.json + images)."""
    import asyncio
    from starlette.background import BackgroundTask
    from common.global_handlers import global_datasets_handler
    ...
    texts_raw = global_new_text_handler._collection.find({"dataset_id": int(dataset_id)})
    texts = [NewText.parse_obj(t) for t in texts_raw]
    if not texts:
        raise HTTPException(status_code=404, detail="No texts in dataset")

    zip_path = await asyncio.to_thread(
        global_new_text_handler.build_cured_export_zip,
        texts, dataset_id, dataset.name,
    )

    safe_name = (dataset.name or str(dataset_id)).replace(" ", "_")
...
async def import_cured_folder(dto: ImportFolderDto):
    ...
    result = await asyncio.to_thread(
        ...
        dto.folder_path, dto.dataset_id,
    )
