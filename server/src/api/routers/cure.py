...
from api.dto.dataset import CreateDatasetDto, RenameDatasetDto, DatasetPreviewDto
from handlers.cure_handler import cure_handler
from handlers.cure_datasets_handler import cure_datasets_handler
...
# ──────────────────────────────────────────────
# Dataset management endpoints (separate from CuReD)
# ──────────────────────────────────────────────


@router.get("/datasets/list")
async def list_cure_datasets() -> list[DatasetPreviewDto]:
    """List all CuRe datasets (separate from CuReD datasets)."""
    datasets = cure_datasets_handler.list_datasets()
    return [
        DatasetPreviewDto(
...
async def create_cure_dataset(dto: CreateDatasetDto) -> int:
    """Create a new CuRe dataset."""
    return cure_datasets_handler.create_dataset(name=dto.name)


@router.patch("/datasets/{dataset_id}/rename")
async def rename_cure_dataset(dataset_id: int, dto: RenameDatasetDto):
    """Rename a CuRe dataset."""
    dataset = cure_datasets_handler.get_dataset(dataset_id)
    if not dataset:
        raise HTTPException(status_code=404, detail="CuRe dataset not found")
    cure_datasets_handler.rename_dataset(dataset_id=dataset_id, name=dto.name)
    return {"updated": True}


@router.delete("/datasets/{dataset_id}")
async def delete_cure_dataset(dataset_id: int):
    """Delete a CuRe dataset."""
    dataset = cure_datasets_handler.get_dataset(dataset_id)
    if not dataset:
        raise HTTPException(status_code=404, detail="CuRe dataset not found")
    cure_datasets_handler.delete_dataset(dataset_id=dataset_id)
    return {"deleted": True}
