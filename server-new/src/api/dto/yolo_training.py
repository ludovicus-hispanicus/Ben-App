...
class DatasetClassInfo(BaseModel):
    """Class info within a dataset listing."""
    id: int
    name: str
    color: Optional[str] = None
...
class AutoAnnotateRequest(BaseModel):
    """Request to start an auto-annotation job."""
    source_project_id: str = Field(..., description="Project ID containing source images")
    model_name: str = Field(..., description="Trained model to use for predictions")
    dataset_name: str = Field(..., min_length=1, max_length=100, description="Name for the new dataset")
    confidence: float = Field(default=0.25, ge=0.01, le=1.0, description="Confidence threshold")
    iou: float = Field(default=0.45, ge=0.1, le=1.0, description="IoU threshold for NMS")
    val_ratio: float = Field(default=0.2, ge=0.0, le=0.5, description="Fraction of images for validation")


class AutoAnnotateStatusResponse(BaseModel):
    """Status of an auto-annotation job."""
    success: bool
    job_id: str
